from email_alert import (alerte_random_forest, alerte_isolation_forest,
                          alerte_gradient_boosting, alerte_kmeans,
                          alerte_critique)

import streamlit as st
st.set_page_config(
    page_title="SIKEN · Salmani Siham & Mokadm Kenza",
    page_icon="🛡️",
    layout="wide",
...
try:
    from Isolation_forest.iso_forest import isolation_forest, anomaly_score
    HAVE_IF = True
    ...
    for p in candidates:
        if os.path.exists(p):
            ...
            if LABEL_COL in df.columns:
                ...
                df["Attack_Type"] = df[LABEL_COL].map(ATTACK_LABELS).fillna(df[LABEL_COL].astype(str))
            else:
                df["Is_Attack"]   = 0
...
def _generate_synthetic(n=500):
    ...
    for i in range(n):
        ...
        rows.append({
            ...
            "Is_Attack"             : int(lbl != 0),
            "Attack_Type"           : ATTACK_LABELS.get(lbl, str(lbl)),
...
def train_all(_X, _y):
    ...
    if HAVE_IF:
        ...
        from Isolation_forest.iso_forest import isolation_forest as _if
...
def score_if(rd):
    ...
    if MODELS['if'] is not None:
        from Isolation_forest.iso_forest import anomaly_score as _as
...
with st.sidebar:
    st.markdown("## SIKEN")
    st.markdown(
        "<small style='color:#3a6a3a;font-family:Share Tech Mono'>SalmaniSiham - Mokadm Kenza</small>",
        unsafe_allow_html=True
    )
    st.markdown("---")
...
# 10. MAIN LAYOUT
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("# SIKEN — Real-Time Threat Intelligence")
st.markdown(
...
with tab_kmeans:
    ...
    with km1:
        ...
        km_dist_ph = st.empty()
    with km2:
        st.markdown("####  K-Means Anomaly Score Over Time")
    ...
    while st.session_state.is_running and st.session_state.row_idx < total_rows:
        ...
        for _ in range(batch_size):
            ...
            if severity == "CRITICAL":
                alerte_critique(severity, attack, src_ip, dst_ip,
                                gb_s, if_s, km_s, rf_s)
            elif severity in ("HIGH", "MEDIUM"):
                if gb_s > gb_threshold:
                    alerte_gradient_boosting(gb_s, src_ip, dst_ip, gb_threshold)
                if if_s > if_threshold:
                    alerte_isolation_forest(if_s, src_ip, dst_ip, if_threshold)
                if km_s > km_threshold:
                    alerte_kmeans(km_s, km_cluster, src_ip, dst_ip, km_threshold)
                if rf_s > rf_threshold:
                    alerte_random_forest(attack, rf_s, src_ip, dst_ip, rf_threshold)
